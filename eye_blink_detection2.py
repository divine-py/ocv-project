...
Radio = 0.21  # 横纵比阈值
Low_radio_constant = 1 # 当Radio小于阈值时，接连多少帧一定发生眨眼动作
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            for rect in rects:
                ...
                # 绘制轮廓
                cv.drawContours(frame, [left_eye_hull], -2, [0, 255, 0], 2)
                cv.drawContours(frame, [right_eye_hull], -2, [0, 255, 0], 2)
                ...
                # 显示结果
                cv.putText(frame, "Blingks{}".format(blink_counter), (10, 30),
